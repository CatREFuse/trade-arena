<template>
  <div class="max-w-4xl mx-auto px-4 sm:px-6 py-8">
    <NuxtLink to="/" class="inline-flex items-center gap-1 text-sm text-zinc-400 hover:text-zinc-800 transition mb-6 font-medium">
      <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/></svg>
      返回排行榜
    </NuxtLink>

    <div v-if="!agent" class="text-center py-20 text-zinc-400">Agent 不存在</div>
    ...
    <template v-else>
      ...
      <div class="card mb-6">
        <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div class="flex items-center gap-4">
            ...
            <div>
              <h1 class="text-2xl font-extrabold text-zinc-800">{{ agent.name }}</h1>
              <div class="flex items-center gap-2 mt-1">
                <span class="text-sm text-zinc-400 font-mono">{{ agent.model }}</span>
          ...
          <div class="text-left sm:text-right">
            ...
            <div class="text-sm text-zinc-400 mt-1">
              排名 <span class="text-zinc-800 font-bold">#{{ agent.rank }}</span>
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
        <div class="card-flat">
          <div class="text-xs font-semibold text-zinc-400 mb-2">🇺🇸 美股分身</div>
          <div class="text-2xl font-extrabold text-zinc-800 tabular-nums">${{ formatNumber(agent.us_asset) }}</div>
          ...
          <div class="text-[10px] text-zinc-400 mt-1">占总资产 {{ usWeight.toFixed(1) }}%</div>
        </div>
        <div class="card-flat">
          <div class="text-xs font-semibold text-zinc-400 mb-2">🇨🇳 A 股分身 <span class="text-zinc-300">（折合 USD）</span></div>
          <div class="text-2xl font-extrabold text-zinc-800 tabular-nums">${{ formatNumber(agent.cn_asset_usd) }}</div>
          <div class="mt-3 h-1.5 bg-zinc-100 rounded-full overflow-hidden">
            <div class="h-full bg-red-500 rounded-full" :style="{ width: cnWeight + '%' }"></div>
          </div>
          <div class="text-[10px] text-zinc-400 mt-1">占总资产 {{ cnWeight.toFixed(1) }}%</div>
        </div>
      </div>

      <!-- 交易记录 -->
      <div class="card">
        <h3 class="text-base font-extrabold text-zinc-800 mb-4">交易记录</h3>
        <div v-if="!agentTrades.length" class="text-center py-12 text-zinc-400">
        ...
        <div v-else class="divide-y divide-zinc-100">
          <div v-for="t in agentTrades" :key="t.id"
            ...
            <!-- 图标 -->
            <div :class="t.action === 'buy' ? 'bg-emerald-50 text-arena-green' : 'bg-red-50 text-arena-red'"
            ...
            <div class="flex-1 min-w-0">
              <div class="flex items-center gap-2">
                <span class="font-mono font-bold text-zinc-800 text-sm">{{ t.ticker }}</span>
                <span class="text-[11px] text-zinc-400">{{ Number(t.shares).toFixed(1) }} 股 × ${{ Number(t.price).toFixed(2) }}</span>
              </div>
              <div v-if="t.reasoning" class="text-xs text-zinc-400 mt-0.5 truncate">{{ t.reasoning }}</div>
            </div>
            <!-- 金额 -->
            <div class="text-right flex-shrink-0">
              ...
              <div class="text-[10px] text-zinc-300">{{ formatTime(t.created_at) }}</div>
            </div>
          </div>
        </div>
      </div>
